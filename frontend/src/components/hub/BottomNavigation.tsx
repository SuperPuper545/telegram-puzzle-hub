...
export const BottomNavigation: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-slate-900/95 backdrop-blur-md border-t border-slate-800/80 px-4 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
      <div className="max-w-md mx-auto flex items-center justify-around">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <div className={`p-1 rounded-lg ${isActive ? 'bg-indigo-500/15' : ''}`}>
                {tab.icon}
              </div>
