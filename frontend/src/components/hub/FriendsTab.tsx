...
export const FriendsTab: React.FC = () => {
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      {/* Hero Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-900/60 via-purple-900/50 to-pink-900/40 border border-purple-500/20 p-5 shadow-xl">
        <div className="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-pink-500/20 rounded-full blur-2xl pointer-events-none" />
        <div className="absolute bottom-0 left-0 -mb-4 -ml-4 w-32 h-32 bg-indigo-500/20 rounded-full blur-2xl pointer-events-none" />
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/20 border border-purple-400/30 text-purple-300 text-xs font-bold mb-2">
              <Gift className="w-3.5 h-3.5 text-purple-300" /> Реферальная программа
            </div>
            ...
            <p className="text-xs text-white/80 mt-1.5 leading-relaxed max-w-[260px]">
              За каждого приглашенного друга вы оба мгновенно получаете по 500 монет на счет!
            </p>
          </div>

          <div className="p-3 rounded-2xl bg-white/10 backdrop-blur-md text-amber-300 shadow-inner">
      ...
      <div className="rounded-3xl bg-tg-secondaryBg border border-[var(--tg-theme-section-separator-color)] p-4 shadow-lg space-y-3">
        ...
        {/* Link box */}
        <div className="flex items-center justify-between p-2.5 rounded-2xl bg-black/20 border border-white/5 text-xs text-tg-text font-mono truncate">
          <span className="truncate mr-2 text-tg-text/90">
            {inviteLink}
          </span>
          <button
            onClick={handleCopy}
            className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/15 active:scale-95 text-xs font-bold text-tg-text transition-all cursor-pointer shrink-0"
          >
            {copied ? (
              <>
        ...
        <button
          onClick={handleShare}
          className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl tg-btn-primary font-black text-sm shadow-xl shadow-indigo-600/30 active:scale-[0.98] transition-all cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="p-3.5 rounded-2xl bg-tg-secondaryBg border border-[var(--tg-theme-section-separator-color)] flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-base font-extrabold text-amber-300">+{totalEarned.toLocaleString()} 🪙</p>
          </div>
        </div>
      </div>

      {/* Invited Friends List */}
      <div className="rounded-3xl bg-tg-secondaryBg border border-[var(--tg-theme-section-separator-color)] p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="space-y-2">
            {referralsList.map((ref) => {
              ...
              return (
                <div
                  key={ref.id}
                  className="flex items-center justify-between p-2.5 rounded-2xl bg-black/15 border border-white/5"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 p-[1.5px] shrink-0">
                      {ref.photoUrl ? (
                        ...
                        <div className="w-full h-full bg-slate-800 rounded-full flex items-center justify-center font-bold text-xs text-indigo-300">
                          {initials}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 text-xs font-black shrink-0">
